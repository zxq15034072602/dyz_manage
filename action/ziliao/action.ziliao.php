...
if($do=='index'){//资料模块首页
   ...
   // var_dump($pArr);
    ...
    //var_dump($mArr);
    //获取康复案例
    $sql="select a.*,b.name as gname,c.name as mname,d.content from ((rv_case as a left join rv_goods as b on a.gid=b.id)left join rv_mendian as c on a.mid=c.id)left join rv_case_detail as d on a.id=d.cid ";
    $db->p_e($sql, array());
    $cArr=$db->fetchAll(); 
     foreach($cArr as $k=>&$v){
