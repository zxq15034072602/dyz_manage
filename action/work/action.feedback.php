...
if ($do == "question") {//提问
    ...
    if ($db->insert(0, 2, "rv_feedback", array(
        ...
        echo '{"code":"200","msg":"问题提交成功 "}';
        exit();
    }
    echo '{"code":"500","msg":"提交失败"}';
    exit();
}elseif($do == "index"){//反馈专区首页
    $uid=$_REQUEST['uid'];
    $pagenum = 5;
    ...
    foreach($aArr as &$value){
       ...
       $sql="select status from rv_answer_greet where aid=$value[aid]";
       $db->p_e($sql, array());
       $value['greet']=$db->fetchRow();
    } 
    
    //显示回答数
    $sql2="select count(*) from rv_answer where qid=?";
    $db->p_e($sql2, array(
        $qid
    ));    
    $hfnum=$db->fetch_count();
    ...
    //获取用户名
    $sql="select name from rv_user where id=$uid";
    $db->p_e($sql, array());
    $name=$db->fetchRow();
    $uname=$name['name'];
        ...
        if($status==0){
            ...
            if($status===false){
                ...
                if( $db->update(0, 1, "rv_guanzhu", array(
                    ...
                    echo '{"code":"200","msg":"您已经关注","status":"1"}';//推送消息
                    exit();                  
                }
            }
        }
    ...
    if(!empty($uid) && !empty($aid)){
        if($status==0){
            if($db->update(0, 1, "rv_answer_greet", array(
                ...
                echo '{"code":"500","msg":"您已取消赞同","status":"0"}';//推送消息
                exit();
            }
        }else{
            ...
            if($status===false){
                if($db->insert(0, 2, "rv_answer_greet", array(
                    ...
                    echo '{"code":"200","msg":"您已赞同","status":"1"}';//推送消息
                    exit();
                }
            }else{
                if( $db->update(0, 1, "rv_answer_greet", array(
                    ...
                    echo '{"code":"200","msg":"您已经赞同","status":"1"}';//推送消息
                    exit();
                }
            }
        }
